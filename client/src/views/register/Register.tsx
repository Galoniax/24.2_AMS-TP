...
import { FaCalendarAlt, FaEnvelope, FaIdCard, FaLock, FaUser } from "react-icons/fa";
...
const Register = () => {
  ...
  const isFormComplete = email && username && dni && birthDate && password && confirmPassword && !emailError && !passwordError && !confirmPasswordError;

  const handleSubmit = () => {
    if (isFormComplete) {
      console.log('Datos del formulario:', { email, username, dni, birthDate, password });
    }
  };
...
return (
    <div className="flex justify-center items-center h-screen w-full bg-slate-100">
      <div className="bg-white shadow-lg rounded-lg p-8 w-[40%]" style={{ minWidth: '375px' }}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            <label className="block mb-1 text-sm mt-2">Fecha de nacimiento</label>
            <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          ...
          <div>
            <label className="block mb-1 text-sm mt-2">Repetir contraseña</label>
            <div className="relative">
              <FaLock className="absolute left-1 top-[5px] text-gray-400" />
              <input 
                type="password" 
